...
var import_core = require("@sonolus/core");
var import_usctool = require("usctool");
const uscToLevelData = (usc, offset = 0) => {
  const entities = [];
  const timeToIntermediates = /* @__PURE__ */ new Map();
  const intermediateToRef = /* @__PURE__ */ new Map();
  const intermediateToEntity = /* @__PURE__ */ new Map();
  let i = 0;
  ...
  const append = (intermediate) => {
    ...
    if (intermediate.sim) {
      const beat = intermediate.data[import_core.EngineArchetypeDataName.Beat];
  ...
  for (const tsGroup of usc.objects) {
    ...
    for (const [index, change] of Object.entries(changes)) {
      tsChangeEntities.push({
        ...
        data: [
          {
            name: import_core.EngineArchetypeDataName.Beat,
            value: change.beat
          },
          {
            name: "timeScale",
            value: change.timeScale === 0 ? 1e-6 : change.timeScale
          },
          tsGroup.changes[+index + 1] === void 0 ? {
            name: "next",
            value: -1
          } : {
            name: "next",
  ...
  for (const intermediates of timeToIntermediates.values()) {
    for (let i2 = 1; i2 < intermediates.length; i2++) {
      append({
        archetype: "SimLine",
        data: {
          a: intermediates[i2 - 1],
          b: intermediates[i2]
        },
        sim: false
      });
...
const bpm = (object, append) => {
  append({
    archetype: import_core.EngineArchetypeName.BpmChange,
    data: {
      [import_core.EngineArchetypeDataName.Beat]: object.beat,
      [import_core.EngineArchetypeDataName.Bpm]: object.bpm
    },
    sim: false
  });
};
const timeScaleGroup = () => void 0;
  ...
  if (object.trace) {
    ...
    if (object.direction) {
      if (object.direction === "none") {
        ...
        intermediate.archetype = object.critical ? "CriticalTraceFlickNote" : "NormalTraceFlickNote";
  ...
  for (const [i, connection] of connections.entries()) {
    if (i === 0) {
      ...
      const ci = {
        archetype,
        data: {
          [import_core.EngineArchetypeDataName.Beat]: connection.beat,
    ...
    if (i === connections.length - 1) {
      ...
      if (connection.judgeType === "none") {
        ...
        if ("direction" in connection) {
          ci.archetype = connection.critical ? "CriticalSlideEndFlickNote" : "NormalSlideEndFlickNote";
    ...
    switch (connection.type) {
      case "tick": {
        ...
        if ("critical" in connection)
          ci.archetype = connection.critical ? "CriticalSlideTickNote" : "NormalSlideTickNote";
        cis.push(ci);
        joints.push(ci);
        break;
      }
      case "attach": {
        ...
        if ("critical" in connection)
          ci.archetype = connection.critical ? "CriticalAttachedSlideTickNote" : "NormalAttachedSlideTickNote";
...
const guide = (object, append) => {
  ...
  for (const [i, joint] of object.midpoints.entries()) {
    ...
    append({
      archetype: "Guide",
      data: {
        color: import_usctool.USCColor[object.color],
        fade: import_usctool.USCFade[object.fade],
        ease: eases[head.ease],
